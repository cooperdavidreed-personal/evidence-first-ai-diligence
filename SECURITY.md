...
- Portable JSON preserves proposal text and named human disposition but deliberately demotes provider identity to `source unverified` on import. Portable policy exceptions are not applied; the receiving browser requires a fresh named policy-owner action. The public browser-local release does not claim cryptographic model-origin or human-authority attestation.
