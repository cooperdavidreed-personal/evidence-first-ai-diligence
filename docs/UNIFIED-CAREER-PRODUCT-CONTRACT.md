...
Northstar's complete golden package must continue to reproduce `$15.9M` LTM revenue, `70.0%` gross margin, `83.6%` ordinary-cohort NRR, `33.3%` ownership, `3.23x` MOIC, and `26.5%` annualized return within declared rounding. Its terminal state is `SCREENING COMPLETE — FURTHER DILIGENCE REQUIRED`; the `83.6%` NRR gate is a visible concern or blocker until an authorized policy owner records an override and rationale. Missing or modified required files continue to yield `NO CALL — PACKAGE INCOMPLETE` and suppress return conclusions.
