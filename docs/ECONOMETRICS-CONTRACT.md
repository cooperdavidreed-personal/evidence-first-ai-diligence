...
| AG-06 | Naive realized-price association — `PREDICTIVE_ASSOCIATION` | Joint HC1 influence delta uncertainty includes the same-account covariance between the observational slope and randomized first stage. The estimate carries zero investment-model credit and the confounding limitation. |
...
| HX-06 | Restricted-randomization optimizer experiment — `CAUSAL_SYNTHETIC_ONLY` | Propose seeded 60-of-120 permutations and accept the first with absolute baseline SMD at or below 0.15, up to 1,000 proposals, without inspecting outcomes. The unadjusted ITT is primary; baseline-adjusted OLS is a labeled precision companion. Dataset rerolls remain zero and are distinct from assignment proposals. |
