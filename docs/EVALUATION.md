# Evaluation plan

The deterministic regression manifest defines 24 executable cases in eight families. A local
result is evidence of declared control behavior only; it is not investment
accuracy, semantic-entailment accuracy, or hosted reliability.

The release target is 16 visible development cases and eight held-out cases,
with held-out inputs hashed before three final repetitions. Deterministic checks
grade schema, paths, hashes, dates, calculations, state transitions, and tool
authority. A named human grades semantic entailment and memo usefulness. A model
judge may advise but cannot be authoritative.
