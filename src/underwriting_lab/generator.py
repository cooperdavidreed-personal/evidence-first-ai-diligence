...
def _atlasgrid(case_root: Path, truth_root: Path, seed: int) -> list[dict[str, Any]]:
    ...
    for month in range(60):
        ...
        fully_burdened_cogs = hosting + implementation + customer_success + credits
    ...
    ltm_normalized = sum(int(row["normalized_ebitda_cents"]) for row in monthly[-12:])
    addbacks = [
        {"item": "Seller adjusted EBITDA", "amount_cents": ltm_normalized + 520_000_000, "treatment": "starting_point"},
        {"item": "Customer-success burden", "amount_cents": -265_000_000, "treatment": "normalize"},
...
def _helios(case_root: Path, truth_root: Path, seed: int) -> list[dict[str, Any]]:
        ...
        monthly.append(
            {
                ...
                "ending_cash_cents": max(0, 4_800_000_000 - sum(int(row["net_burn_cents"]) for row in monthly)),
            }
        )
    rows = _write_csv(case_root / "data/monthly_pnl.csv", list(monthly[0]), monthly)
    ...
    cap_table = {
        ...
        "cash_at_cutoff_cents": 1_900_000_000,
    }
