...
test("local Version 2 changes propagate and require explicit accept reject or defer", async ({page}, testInfo: TestInfo) => {
  ...
  await memoDownload.click();
  const memoFile = testInfo.outputPath("northstar-v2-accepted-ic-memo.html");
  await (await memoDownloadPromise).saveAs(memoFile);
  const memoHtml = readFileSync(memoFile, "utf8");
  expect(memoHtml).toContain("V2 admitted package case");
  expect(memoHtml).toContain("78.6% cohort retention proxy");
