...
const outputSchema = jsonSchema<{
  ...
  challenges: {type: "array", minItems: 1, maxItems: 3, items: {type: "object", additionalProperties: false, properties: {claim: {type: "string", maxLength: 500}, evidence_refs: {type: "array", minItems: 1, maxItems: 12, items: {type: "string"}}, severity: {type: "string", enum: ["HIGH", "MEDIUM", "LOW"]}, management_question: {type: "string", maxLength: 500}}, required: ["claim", "evidence_refs", "severity", "management_question"]}},
...
export function validateChallengeOutput(raw: unknown, allowedEvidenceIds: Set<string>): ChallengeOutput {
  ...
  const challenges = Array.isArray(value.challenges) && value.challenges.length >= 1 && value.challenges.length <= 3 ? value.challenges.map((candidate) => {if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) throw new Error("Generated challenge is invalid"); const item = candidate as Record<string, unknown>; const claim = string(item.claim, 500), management_question = string(item.management_question, 500), severity = item.severity; if (!claim || !management_question || !["HIGH", "MEDIUM", "LOW"].includes(String(severity))) throw new Error("Generated challenge is invalid"); return {claim, management_question, severity: severity as "HIGH" | "MEDIUM" | "LOW", evidence_refs: refs(item.evidence_refs)};}) : (() => {throw new Error("Generated challenges are invalid");})();
