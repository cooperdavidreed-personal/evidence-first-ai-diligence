...
export function operatingText(deal){const r=buildOperatingReview(deal);if(!r)return [];return ['Monthly operating review',r.period+' · '+r.scenario+' · '+r.currency+' · evidence available through '+r.cutoff,...r.lines.map(l=>l.metric+': actual '+(l.actual?.value??'unavailable')+'; forecast '+(l.forecast?.value??'unavailable')+'; variance '+(l.delta??'not comparable')+'; '+l.status+'; sources '+[l.actual,l.forecast].filter(Boolean).map(x=>x.ref).join(', ')),...r.derived.map(d=>d.label+': '+(d.actual??'unavailable')+(d.label.includes('%')?'%':' '+r.currency)+'. '+d.explanation+' Sources: '+d.refs.join(', ')),'Minimum cash assumption: '+r.minimumCash+' '+r.currency+'; owner '+r.owner,...r.concerns.map(c=>'Requires attention: '+c.question+' '+c.why),...r.exclusions.map(e=>'Excluded: '+e.sourceId+' — '+e.reason)];}
