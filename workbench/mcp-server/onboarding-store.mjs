...
export function onboardingMethods(db, now=()=>Date.now()) {
  ...
  return {
    onboardingStatus(){const row=db.prepare('SELECT expires,verified,completed FROM desktop_onboarding WHERE id=1').get();return {verified:!!row.verified,expires:row.expires,completed:!!row.completed};},
