...
try{
 ...
 assert.match(html,/__DESK_DESKTOP__=true/);
 ...
 const files=unzipSync(await readFile(join(installed.root,'Underwriting Desk.mcpb')));const manifest=JSON.parse(Buffer.from(files['manifest.json']).toString());assert.equal(manifest.server.type,'binary');const connector=join(temp,'connector');await writeFile(connector,files['server/connector']);await chmod(connector,0o700);
