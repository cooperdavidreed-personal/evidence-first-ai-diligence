...
const manifest={manifest_version:'0.3',name:'underwriting-desk',display_name:'Underwriting Desk',version:'0.3.2',description:'Connect Claude to your local investment workspace. Read only analyst-released evidence and propose work for review.',author:{name:'Cooper Reed'},server:{type:'binary',entry_point:'server/connector',mcp_config:{command:'${__dirname}/server/connector',args:[],platform_overrides:{win32:{command:'${__dirname}/server/connector.exe'}}}},compatibility:{platforms:['darwin','win32']},tools_generated:true};
...
artifacts.push({file:'Underwriting Desk.mcpb',bytes:extension.length,sha256:hash(extension),nativeTested:false});await writeFile(join(out,'artifacts.json'),JSON.stringify({version:'0.3.2-local',nodeVersion:version,signed:false,artifacts},null,2));console.log(JSON.stringify(artifacts,null,2));
