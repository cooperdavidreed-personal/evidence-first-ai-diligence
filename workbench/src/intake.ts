...
function sourceMoney(cents: number) { return new Intl.NumberFormat("en-US", {style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2}).format(cents / 100); }
...
function analyze(deal: DealInput, monthly: MonthlyRow[], customers: CustomerRow[], policyProfile: PolicyProfile): QuickAnalysis {
  ...
  const sourcePreviews: QuickSourcePreview[] = [
    ...
    {sourceFile: "monthly_financials.csv", classification: "SOURCE_FACT", title: "Latest twelve monthly rows", period: `${ltm[0].period} to ${ltm.at(-1)!.period}`, excerpt: [{label: "Rows", value: String(ltm.length)}, {label: "Revenue total", value: sourceMoney(ltmRevenueCents)}, {label: "Cost of revenue", value: sourceMoney(ltmCostCents)}, {label: "Latest month revenue", value: sourceMoney(ltm.at(-1)!.revenueCents)}, {label: "Recent signed net burn", value: sourceMoney(recentNetBurnCents)}], rows: ltm.map((row) => ({dataRow: row.sourceRow, cells: [{label: "Period", value: row.period}, {label: "Revenue", value: sourceMoney(row.revenueCents)}, {label: "Cost of revenue", value: sourceMoney(row.costOfRevenueCents)}, {label: "Operating expense", value: sourceMoney(row.operatingExpenseCents)}]}))},
    {sourceFile: "customer_arr.csv", classification: "SOURCE_FACT", title: "Opening-customer retention cohort", period: `${basePeriod} to ${latestPeriod}`, excerpt: [{label: "Opening customers", value: String(base.size)}, {label: "Opening ARR", value: sourceMoney(baseArr)}, {label: "Ending ARR for opening IDs", value: sourceMoney(endingArr)}, {label: "Elapsed interval", value: `${cohortElapsedMonths} months`}, {label: "Cohort retention proxy", value: percent(ordinaryNrr)}], rows: customers.filter((row) => (row.period === basePeriod && row.arrCents > 0) || (row.period === latestPeriod && base.has(row.customerId))).map((row) => ({dataRow: row.sourceRow, cells: [{label: "Customer", value: row.customerId}, {label: "Period", value: row.period}, {label: "ARR", value: sourceMoney(row.arrCents)}]}))},
