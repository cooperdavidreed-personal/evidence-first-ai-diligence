...
export function canonicalChallengePayload(evidence: SelectedEvidence[]) {
  return JSON.stringify({job: "challenge_selected_evidence", evidence: evidence.map(({id, title, displayValue, summary}) => ({id, title, displayValue, summary})), output_contract: "underwriting-evidence-challenge/v1"});
}

export async function digestChallengePayload(evidence: SelectedEvidence[]) {
  return digestChallengePayloadSync(evidence);
}
...
export function digestChallengePayloadSync(evidence: SelectedEvidence[]) {
  return digestTextSync(canonicalChallengePayload(evidence));
}
...
export function validateSelectedEvidence(evidence: SelectedEvidence[]) {
  ...
  if (new TextEncoder().encode(canonicalChallengePayload(evidence)).byteLength > 12_000) throw new Error("Selected evidence exceeds the 12,000-byte model-review boundary");
}

export function validateModelOutput(raw: unknown, evidence: SelectedEvidence[], requestDigestSha256: string, responseDigestSha256: string) {
  ...
  if (raw.request_digest_sha256 !== requestDigestSha256) return {proposals, droppedItems: 1};
  ...
  challenges.forEach((item, index) => {
    ...
    proposals.push({proposalId: proposalId("CHALLENGE", index, evidenceRefs, requestDigestSha256, responseDigestSha256), kind: "CHALLENGE", state: "PROPOSED", title: claim, body: question, evidenceRefs, requestEvidence: evidence.map((item) => ({...item})), severity, requestDigestSha256, responseDigestSha256, modelFamily, limitations});
  });
  ...
  gaps.forEach((item, index) => {
    ...
    proposals.push({proposalId: proposalId("DILIGENCE_GAP", index, evidenceRefs, requestDigestSha256, responseDigestSha256), kind: "DILIGENCE_GAP", state: "PROPOSED", title, body, evidenceRefs, requestEvidence: evidence.map((item) => ({...item})), proposedOwner, requestDigestSha256, responseDigestSha256, modelFamily, limitations});
  });
  ...
  drafts.forEach((item, index) => {
    ...
    proposals.push({proposalId: proposalId("MEMO_DRAFT", index, evidenceRefs, requestDigestSha256, responseDigestSha256), kind: "MEMO_DRAFT", state: "PROPOSED", title: `Draft ${section}`, body, evidenceRefs, requestEvidence: evidence.map((item) => ({...item})), memoSection: section, requestDigestSha256, responseDigestSha256, modelFamily, limitations});
  });
...
export async function runEvidenceChallenge(evidence: SelectedEvidence[], transport?: ModelTransport): Promise<ModelReviewResult> {
  ...
  try {
    const requestDigestSha256 = await digestChallengePayload(evidence);
    const raw = await transport({job: "challenge_selected_evidence", evidence: evidence.map((item) => ({...item})), output_contract: "underwriting-evidence-challenge/v1", request_digest_sha256: requestDigestSha256});
    const responseDigestSha256 = await digestResponseEnvelope(raw);
    const validated = validateModelOutput(raw, evidence, requestDigestSha256, responseDigestSha256);
