...
export const SNOWFLAKE_SOURCES: PublicRecordSource[] = [
  {id: "snow-s1", title: "Initial registration statement", form: "S-1", filedAt: "2020-08-24T16:06:56Z", url: "https://www.sec.gov/Archives/edgar/data/1640147/000162828020013010/snowflakes-1.htm", state: "ADMITTED", use: "Audited financial statements, business model, risks and metric definitions."},
  {id: "snow-s1a", title: "Pre-IPO amendment", form: "S-1/A", filedAt: "2020-09-14T06:05:32Z", url: "https://www.sec.gov/Archives/edgar/data/1640147/000162828020013518/snowflakes-1a2.htm", state: "ADMITTED", use: "Latest available price range and operating disclosures at the cutoff."},
...
export const SNOWFLAKE_FACTS = [
  {label: "Six-month product revenue", value: "$227.0M", comparison: "Six months ended July 31, 2020; $100.6M in the comparable 2019 period", classification: "Reported fact", sourceId: "snow-s1a"},
  {label: "Product revenue growth", value: `${((227 / 100.6 - 1) * 100).toFixed(1)}%`, comparison: "Derived: $227.0M ÷ $100.6M − 1", classification: "Derived calculation", sourceId: "snow-s1a"},
  {label: "Customers", value: "3,117", comparison: "As of July 31, 2020; 1,547 as of July 31, 2019", classification: "Reported fact", sourceId: "snow-s1a"},
  {label: "Net revenue retention", value: "158%", comparison: "As of July 31, 2020; company-defined capacity-contract cohort. This is not gross retention.", classification: "Reported fact", sourceId: "snow-s1a"},
  {label: "Remaining performance obligations", value: "$688.2M", comparison: "As of July 31, 2020; $221.1M one year earlier. Contracted obligations are not recognized revenue.", classification: "Reported fact", sourceId: "snow-s1a"},
...
export function PublicRecordCase({onDeals}: {onDeals: () => void}) {
  ...
  return <main className="public-record-page" id="main-content">
    ...
    <div className="public-record-grid"><section className="panel"><div className="section-heading"><div><p className="eyebrow">Evidence available at cutoff</p><h2>Operating signal</h2></div><span>SEC filings only</span></div><div className="public-fact-table">{SNOWFLAKE_FACTS.map((fact) => <article key={fact.label}><div><strong>{fact.label}</strong><small>{fact.classification}</small></div><b>{fact.value}</b><p>{fact.comparison}</p><details><summary>Interpret this figure</summary><p>{fact.label === "Indicative offer midpoint" ? "Cover page: indicative range of $100–$110 per share. Midpoint is arithmetic, not an entry price or valuation opinion." : fact.label === "Net revenue retention" ? "Find Net Revenue Retention Rate in the key business metrics discussion. Expansion can offset lost consumption; customer-level churn cannot be reconstructed from this aggregate." : fact.label === "Remaining performance obligations" ? "Find Remaining Performance Obligations in the key business metrics discussion. Contract timing and consumption affect conversion; do not add this balance to revenue." : "Find product revenue and customer disclosures in the operating discussion. Verify the period and definition before comparing with another company."}</p></details><a href={SNOWFLAKE_SOURCES.find((source) => source.id === fact.sourceId)!.url} target="_blank" rel="noreferrer">Read source document ↗</a></article>)}</div></section>
      <aside className="panel public-record-judgment"><p className="eyebrow">Decision meaning</p><h2>Exceptional growth; price discipline remains unproved</h2><p>The admitted record supports unusually strong consumption expansion, customer growth and contracted demand. It does not support a complete ownership, dilution, downside or return model at this cutoff.</p><h3>Contradictory evidence to investigate</h3><p>High expansion does not establish durable gross retention, and contracted demand does not establish cash conversion or profitability. These gaps prevent a return conclusion from following mechanically from growth.</p><h3>What must be true</h3><ul><li>Consumption expansion must remain durable as the base scales.</li><li>RPO must convert without masking demand timing or rollover risk.</li><li>Unit economics must improve enough to justify the indicative valuation.</li></ul><h3>Missing private evidence</h3><ul><li>Customer-level cohorts, gross retention and concentration.</li><li>Pipeline quality, cloud commitments and contract-level economics.</li><li>Board plan, cap-table detail and negotiated allocation.</li></ul><h3>Next committee action</h3><p>Commission a cohort-quality and unit-economics review. Keep pricing and allocation conditional until ownership, dilution and downside assumptions can be defended.</p><p className="public-record-boundary">This is a historical public-information screen, not sponsor diligence and not an investment recommendation.</p></aside></div>
