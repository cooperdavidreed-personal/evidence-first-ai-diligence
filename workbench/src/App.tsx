...
function evidenceReferenceLabels(caseData: CaseData) {
  return Object.fromEntries([
    ...caseData.metricRegistry.map((metric) => [metric.metric_id, metric.label] as const),
    ...caseData.analyses.map((analysis) => [analysis.analysis_id, analysis.question] as const),
    ...caseData.artifacts.map((artifact) => [artifact.artifact_id, friendlyFileName(artifact.path)] as const),
  ]);
}
...
function Diligence({caseData, modelTransport, connection, proposals, onProposalsChange}: {caseData: CaseData; modelTransport?: ModelTransport; connection: ConnectionState | null; proposals: ModelProposal[]; onProposalsChange: (proposals: ModelProposal[]) => void}) {
  ...
  return (
    <div className="view-stack">
      ...
      <ModelReviewPanel connection={connection} transport={modelTransport} proposals={proposals} onProposalsChange={onProposalsChange} evidence={caseData.summaryMetrics.map((metric) => ({id: metric.metric_id, title: metric.label, displayValue: metric.value, summary: metric.detail}))} referenceLabels={evidenceReferenceLabels(caseData)} />
    </div>
  );
}
