...
import type {WorkspaceScenarioContract, WorkspaceSeed} from "./workspace-state";
...
function dealMeta(caseId: CaseId) {
  return caseId === "atlasgrid"
    ? {owner: "Buyout team", stage: "Pre-IC", posture: "REPRICE", blockers: 5, next: "Validate full-cohort retention and price below the seller ask", updated: "2026-08-29"}
    : {owner: "Growth team", stage: "Diligence", posture: "HOLD", blockers: 6, next: "Reconcile pipeline, provider costs and executed financing terms", updated: "2026-08-29"};
}

function DealList({onOpen, onNew, onConnect, connection, localDeal, onOpenLocal, onImportLocal, importNotice}: {onOpen: (caseId: CaseId) => void; onNew: () => void; onConnect: () => void; connection: ConnectionState | null; localDeal: IntakeResult | null; onOpenLocal: () => void; onImportLocal: (file?: File) => void; importNotice: string}) {
  const dealButton = (deal: typeof caseCatalog[number]) => {
    const meta = dealMeta(deal.caseId);
    return <button type="button" className="deal-row" key={deal.caseId} aria-label={`Open ${deal.company} — ${meta.posture}; ${meta.blockers} blockers; next: ${meta.next}`} onClick={() => onOpen(deal.caseId)}><span><strong>{deal.company}</strong><small>{deal.investmentQuestion}</small></span><span>{deal.caseType}</span><span>{meta.owner}</span><span>{meta.stage}</span><span className={`posture posture-${meta.posture.toLowerCase()}`}>{meta.posture}</span><span>{meta.blockers}</span><span>{formatHumanDate(`${meta.updated}T12:00:00Z`)}</span><span>{meta.next}</span></button>;
  };
...
function assumptionsFor(caseData: CaseData): AssumptionDefinition[] {
  ...
  return [
    {id: "growth", label: "Annual revenue growth", value: percent(bridge.annual_revenue_growth), owner: engine.distribution.priors.owner, basis: "Analyst scenario", consequence: "Changes terminal revenue and the operating exit bridge.", status: engine.distribution.priors.approval_status},
    {id: "exit-multiple", label: "Exit revenue multiple", value: `${Number(bridge.exit_revenue_multiple).toFixed(1)}x`, owner: engine.distribution.priors.owner, basis: "Analyst scenario", consequence: "Changes enterprise value and investor proceeds.", status: engine.distribution.priors.approval_status},
...
function DecisionRail({caseData, view, state}: {caseData: CaseData; view: DealView; state: ReturnType<typeof useDealWorkspace>["state"]}) {
  ...
  return <aside className="decision-rail" aria-label="Decision status"><header><span>Current posture</span><strong>{caseData.decision.decision}</strong><p>Working recommendation · human review required</p></header><dl><div><dt>View</dt><dd>{viewLabels[view]}</dd></div><div><dt>Scenario</dt><dd>{working ? "Unapproved what-if" : "Canonical case"}</dd></div><div><dt>Open blockers</dt><dd>{unresolved.length}</dd></div><div><dt>Policy state</dt><dd>{policy ? `${policy.status.toLowerCase()} · ${policy.lastReviewed ?? "not reviewed"}` : "Retained decision tests · IC review pending"}</dd></div></dl><section><span>Primary blocker</span><strong>{unresolved[0]?.title ?? "No unresolved issue"}</strong><p>{unresolved[0]?.decisionImpact ?? "All recorded issues are resolved."}</p></section><section><span>Next action</span><strong>{caseData.decision.path_to_yes[0]}</strong></section><footer>IC decision pending</footer></aside>;
}
...
function Diligence({caseData, state, update, modelTransport, connection, openMetric}: {caseData: CaseData; state: ReturnType<typeof useDealWorkspace>["state"]; update: ReturnType<typeof useDealWorkspace>["update"]; modelTransport?: ModelTransport; connection: ConnectionState | null; openMetric: (metric: Metric, trigger: HTMLElement) => void}) {
  ...
  const evidence = caseData.summaryMetrics.slice(0, 8).map((metric) => {
    const record = caseData.metricRegistry.find((item) => item.metric_id === metric.metric_id);
    const sourceCount = record?.source_locator_ids.length ?? metric.lineage.length;
    return {id: metric.metric_id, title: metric.label, displayValue: metric.value, summary: `${metric.detail} ${sourceCount} linked source ${sourceCount === 1 ? "record" : "records"}; ${metric.classification.toLowerCase().replaceAll("_", " ")}.`};
  });
