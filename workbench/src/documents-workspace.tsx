...
export function DocumentsWorkspace({caseData, openMetric}: {caseData: CaseData; openMetric: OpenMetric}) {
  ...
  return <div className="documents-workspace">
    ...
    <section className="document-preview" aria-live="polite">{selected ? <><header><div><p className="eyebrow">Evidence preview</p><h2>{friendly(selected.path)}</h2><p>{selected.sourceClass} · {selected.path.split("/").at(-1)}</p></div>{selected.locators[0]?.published_path ? <a href={selected.locators[0].published_path} target="_blank" rel="noreferrer">Open complete synthetic source</a> : <span>Complete file retained in the public source package</span>}</header><div className="preview-summary"><div><span>Source type</span><strong>{selected.sourceClass}</strong></div><div><span>Source rows</span><strong>{selected.rows.toLocaleString()}</strong></div><div><span>Linked calculations</span><strong>{linkedMetrics.length}</strong></div></div><div className="excerpt-list">{previewLocators.length ? previewLocators.map((locator: SourceLocator) => <article key={locator.locator_id}><div className="excerpt-heading"><span>{formatHumanDate(locator.period)}</span><small>{locator.locator_kind === "CSV_CELLS" ? "Exact rows and cells" : locator.locator_kind === "JSON_POINTERS" ? "Exact selected fields" : "Exact text excerpt"}</small></div><EvidenceExcerpt locator={locator} /></article>) : <p className="empty-copy">The source is retained, but no granular preview is registered. It does not support a displayed calculation in this view.</p>}</div>{linkedMetrics.length ? <aside className="linked-calculations"><h3>Calculations using this source</h3>{linkedMetrics.map((registry) => {const metric = metricFrom(caseData, registry.metric_id)!; return <button key={registry.metric_id} type="button" onClick={(event) => openMetric(metric, event.currentTarget)}><span>{financeLabel(registry.label)}</span><strong>{cleanMetricDisplay(registry.display_value)}</strong><small>Trace calculation</small></button>;})}</aside> : null}<details className="technical-record"><summary>Reproduction detail</summary><dl><div><dt>Source digest</dt><dd><code>{selected.sha256}</code></dd></div><div><dt>Artifact id</dt><dd><code>{selected.artifact_id}</code></dd></div><div><dt>Case analysis</dt><dd><code>{caseData.analysis_sha256}</code></dd></div></dl></details></> : <p>No retained source or excerpt matches the current search.</p>}</section>
  </div>;
}
