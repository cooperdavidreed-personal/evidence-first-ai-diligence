...
export function DiligenceWorklist({state, update}: {state: DealWorkspaceState; update: WorkspaceUpdate}) {
...
export function EditableMemo({state, update, title, subtitle, scenarioSummary}: {state: DealWorkspaceState; update: WorkspaceUpdate; title: string; subtitle: string; scenarioSummary: ScenarioMemoSummary}) {
  ...
  const setSection = (sectionId: string, body: string) => {
    ...
    update({memoSections: state.memoSections.map((section) => section.sectionId === sectionId ? {...section, body, provenance: section.provenance === "DETERMINISTIC_ANALYSIS" ? "ANALYST_JUDGMENT" : section.provenance, sourceProvenance: section.sourceProvenance ?? section.provenance, sourceBody: section.sourceBody ?? section.body, updatedBy: editor.trim(), updatedAt: new Date().toISOString()} : section)});
  };
  ...
  return <div className="memo-workspace"><section className="memo-editor"><header><div><span>Investment committee working draft</span><h2>{title}</h2><p>{subtitle}</p></div><label><span>Editor</span><input value={editor} maxLength={120} onChange={(event) => setEditor(event.target.value)} placeholder="Named editor required to revise" /></label></header><section className="memo-scenario-summary" aria-label="Scenario represented in this memo"><div><span>{scenarioSummary.state}</span><h3>{scenarioSummary.label}</h3></div><strong>{scenarioSummary.returnLine}</strong><p>{scenarioSummary.detail}</p></section>{state.memoSections.map((section) => <article key={section.sectionId}><div className="memo-section-heading"><div><h3>{section.title}</h3><small>{section.provenance === "DETERMINISTIC_ANALYSIS" ? "Calculated baseline" : section.provenance === "ANALYST_JUDGMENT" ? section.sourceProvenance === "DETERMINISTIC_ANALYSIS" ? "Analyst revision · calculated baseline preserved" : "Analyst judgment" : "Accepted model proposal"}</small></div><span>{section.updatedBy} · {formatHumanDate(section.updatedAt)}</span></div><textarea className="memo-screen-editor" maxLength={8000} disabled={!editor.trim()} aria-label={`${section.title} memo section`} value={section.body} onChange={(event) => setSection(section.sectionId, event.target.value)} /><p className="memo-print-body">{section.body}</p>{section.sourceBody ? <details><summary>Original source text</summary><p>{section.sourceBody}</p></details> : null}</article>)}{acceptedDrafts.length ? <aside className="accepted-drafts"><h3>Accepted proposals ready for the memo</h3>{acceptedDrafts.map((proposal) => <div key={proposal.proposalId}><p>{proposal.body}</p><button type="button" onClick={() => addProposal(proposal)} disabled={!editor.trim()}>Add with provenance</button></div>)}</aside> : null}<footer>Working draft · IC decision pending · Illustrative public data</footer></section><div className="memo-actions"><button className="primary-button" type="button" onClick={() => window.print()}>Export IC memo</button><p>Model language enters only after named human acceptance and a named editor adds it.</p></div></div>;
}
