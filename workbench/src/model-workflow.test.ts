...
describe("controlled evidence challenge", () => {
  it("is honestly unavailable without a configured transport", async () => {
    const result = await runEvidenceChallenge(dealId, evidence);
  ...
  it("produces only structured evidence-linked proposals", async () => {
    const transport = vi.fn(async (request: ModelChallengeRequest) => ({
      deal_id: request.deal_id, request_digest_sha256: request.request_digest_sha256,
    ...
    const result = await runEvidenceChallenge(dealId, evidence, transport);
  ...
  it("drops uncited and unknown-reference items and reports the count", async () => {
    const result = await runEvidenceChallenge(dealId, evidence, async (request) => ({
      deal_id: request.deal_id, request_digest_sha256: request.request_digest_sha256,
  ...
  it("does not mutate the canonical analytical case", async () => {
    ...
    const result = await runEvidenceChallenge(dealId, evidence, async (request) => ({deal_id: request.deal_id, request_digest_sha256: request.request_digest_sha256, challenges: [{claim: "Challenge", evidence_refs: ["metric-runway"], severity: "MEDIUM", management_question: "Test it"}], gaps: [], memo_drafts: []}));
  ...
  it("preserves the original model draft when a named reviewer edits it", async () => {
    const result = await runEvidenceChallenge(dealId, evidence, async (request) => ({deal_id: request.deal_id, request_digest_sha256: request.request_digest_sha256, challenges: [{claim: "Challenge", evidence_refs: ["metric-runway"], severity: "MEDIUM", management_question: "Which committed costs are absent?"}], gaps: [], memo_drafts: []}));
  ...
  it("fails before transport when evidence exceeds the selected-evidence contract", async () => {
    ...
    const result = await runEvidenceChallenge(dealId, [], transport);
    expect(result.status).toBe("FAILED");
    expect(transport).not.toHaveBeenCalled();
  });

  it("rejects a response that omits the exact request digest", async () => {
    const result = await runEvidenceChallenge(dealId, evidence, async () => ({deal_id: dealId, challenges: [{claim: "Unbound", evidence_refs: ["metric-runway"], severity: "HIGH", management_question: "Why?"}], gaps: [], memo_drafts: []}));
    expect(result.proposals).toEqual([]);
