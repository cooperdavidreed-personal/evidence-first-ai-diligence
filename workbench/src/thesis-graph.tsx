  ...
  return <div className="dag-shell">
    ...
    <div className="dag-scroll" tabIndex={0} aria-label="Scrollable thesis dependency graph">
      <svg className="thesis-dag" viewBox={`0 0 1040 ${height}`} role="tree" aria-label="Evidence to decision dependency graph">
        ...
        <g className="dag-nodes">{positioned.map((node: PositionedNode) => <g key={node.node_id} transform={`translate(${node.x} ${node.y})`} role="treeitem" tabIndex={0} aria-selected={selected === node.node_id} className={`dag-node ${node.kind.toLowerCase()} ${active.has(node.node_id) ? "active" : "muted"}`} onClick={() => choose(node.node_id)} onKeyDown={(event) => {if (event.key === "Enter" || event.key === " ") {event.preventDefault(); choose(node.node_id);}}}><rect width="275" height="64" rx="2" /><text x="12" y="18" className="dag-kind">{node.kind}</text><foreignObject x="12" y="25" width="251" height="34"><div className="dag-label">{node.label}</div></foreignObject><title>{node.label} · {node.status}</title></g>)}</g>
      </svg>
    </div>
