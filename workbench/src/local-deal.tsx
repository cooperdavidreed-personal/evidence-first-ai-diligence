...
function LocalDecisionTests({analysis}: {analysis: QuickAnalysis}) {
  return <section className="panel"><div className="section-heading"><div><p className="eyebrow">Declared policy</p><h2>Decision tests</h2></div><span>{analysis.tests.filter((test) => test.status === "MISSES").length} misses</span></div><div className="table-wrap" tabIndex={0} aria-label="Scrollable decision tests"><table><thead><tr><th>Test</th><th>Observed</th><th>Required</th><th>Result</th></tr></thead><tbody>{analysis.tests.map((test) => <tr key={test.label}><td>{test.label}</td><td>{test.observed}</td><td>{test.required}</td><td><span className={`status status-${test.status.toLowerCase()}`}>{stateLabel(test.status)}</span></td></tr>)}</tbody></table></div></section>;
}

function LocalOverview({result}: {result: IntakeResult}) {
  const analysis = result.analysis!;
  return <div className="view-stack"><section className="decision-panel"><div className="decision-copy"><p className="eyebrow">Analytical posture</p><h2>{result.posture}</h2><p>{result.rationale}</p></div><div className="approval-state"><span>IC approval</span><strong>Not requested</strong><span>Quick Package only</span></div></section><section className="first-read-grid"><article><p className="eyebrow">Return scenario</p><h3>{analysis.grossMoic.toFixed(2)}x</h3><p>Gross multiple · {`${(analysis.annualizedGrossReturn * 100).toFixed(1)}%`} annualized</p></article><article><p className="eyebrow">Operating quality</p><h3>{(analysis.ordinaryNrr * 100).toFixed(1)}% NRR</h3><p>Fixed ordinary cohort from uploaded customer ARR.</p></article><article><p className="eyebrow">Financing risk</p><h3>{analysis.runwayMonths === null ? "Cash generative" : `${analysis.runwayMonths.toFixed(1)} months`}</h3><p>Cash divided by recent positive net burn.</p></article></section><LocalDecisionTests analysis={analysis} /><div className="local-data-note"><strong>Analytical posture only.</strong> This is not an investment recommendation, approval, or full-fidelity underwriting result.</div></div>;
}

function LocalFinancials({result}: {result: IntakeResult}) {
  const analysis = result.analysis!;
  return <div className="view-stack"><section className="metric-grid">{analysis.metrics.map((metric) => <article className="metric-card" key={metric.id}><span>{metric.label}</span><strong>{metric.display}</strong><p>{metric.meaning}</p></article>)}</section><section className="panel"><div className="section-heading"><div><p className="eyebrow">Declared scenario</p><h2>Exit and ownership bridge</h2></div></div><div className="table-wrap" tabIndex={0} aria-label="Scrollable exit and ownership bridge"><table><tbody><tr><th>LTM revenue</th><td>{money(analysis.ltmRevenueCents)}</td></tr><tr><th>Terminal revenue scenario</th><td>{money(analysis.terminalRevenueCents)}</td></tr><tr><th>Exit equity scenario</th><td>{money(analysis.exitEquityCents)}</td></tr><tr><th>Post-money ownership</th><td>{(analysis.postMoneyOwnership * 100).toFixed(1)}%</td></tr><tr><th>Gross multiple</th><td>{analysis.grossMoic.toFixed(2)}x</td></tr><tr><th>Annualized gross return</th><td>{(analysis.annualizedGrossReturn * 100).toFixed(1)}%</td></tr></tbody></table></div><details className="method-disclosure"><summary>View calculation limits</summary><p>The exit is debt- and cash-neutral. No preferences, option-pool refresh, later-round dilution, fees, taxes, econometrics, or stochastic paths are modeled in the Quick Package.</p></details></section></div>;
}

function LocalDiligence({result, modelTransport, connection, proposals, onProposalsChange}: {result: IntakeResult; modelTransport?: ModelTransport; connection: ConnectionState | null; proposals: ModelProposal[]; onProposalsChange: (proposals: ModelProposal[]) => void}) {
  const analysis = result.analysis!; const misses = analysis.tests.filter((test) => test.status === "MISSES");
  return <div className="view-stack"><section className="panel"><div className="section-heading"><div><p className="eyebrow">Package worklist</p><h2>{misses.length ? `${misses.length} declared tests need attention` : "No declared threshold miss"}</h2></div><span>Quick Package</span></div>{misses.length ? <div className="issue-list">{misses.map((test) => <article key={test.label}><div><span className="status status-misses">Misses</span><span>blocking</span></div><h3>{test.label}</h3><p>{test.observed}; required {test.required.toLowerCase()}.</p><footer><span>Owner</span><strong>{result.deal!.analystOwner}</strong></footer></article>)}</div> : <p>All three declared thresholds clear. Document quality, customer contracts, revenue recognition, and financing terms still require full diligence.</p>}</section><ModelReviewPanel connection={connection} transport={modelTransport} proposals={proposals} onProposalsChange={onProposalsChange} evidence={analysis.metrics.map((metric) => ({id: metric.id, title: metric.label, displayValue: metric.display, summary: metric.meaning}))} /></div>;
}

function LocalDocuments({result}: {result: IntakeResult}) {
  return <div className="view-stack"><section className="panel"><div className="section-heading"><div><p className="eyebrow">Local package</p><h2>Package status</h2></div><span>In memory only</span></div><div className="document-list">{result.files.map((file, index) => <article key={`${file.name}-${index}`}><div><h3>{file.name.replaceAll("_", " ").replace(/\.[^.]+$/, "")}</h3><p>{file.detail}</p>{file.mappings?.length ? <ul className="mapping-list">{file.mappings.map((mapping) => <li key={`${mapping.from}-${mapping.to}`}>{mapping.from} → {mapping.to}</li>)}</ul> : null}</div><span className={`status status-${file.state.toLowerCase()}`}>{stateLabel(file.state)}</span></article>)}</div></section><div className="local-data-note"><strong>No persistence.</strong> Refreshing this tab clears the selected file objects and computed deal.</div></div>;
}

function LocalMemo({result, proposals}: {result: IntakeResult; proposals: ModelProposal[]}) {
  const analysis = result.analysis!;
  const acceptedDrafts = proposals.filter((proposal) => proposal.kind === "MEMO_DRAFT" && proposal.state === "ACCEPTED"); const reviewed = proposals.filter((proposal) => proposal.state !== "PROPOSED");
  return <div className="memo-page"><section className="memo-sheet"><header><div><span>Investment committee draft</span><h2>{result.deal!.company}</h2><p>Does the supported package clear its declared return and runway thresholds?</p></div><span className="quiet-chip">Browser-local</span></header><article><span className="source-tag">Engine</span><h3>Analytical posture</h3><p>{result.rationale}</p></article><article><span className="source-tag">Engine</span><h3>Economics</h3><p>LTM revenue {money(analysis.ltmRevenueCents)} · gross margin {(analysis.grossMargin * 100).toFixed(1)}% · ordinary-cohort NRR {(analysis.ordinaryNrr * 100).toFixed(1)}% · gross multiple {analysis.grossMoic.toFixed(2)}x.</p></article><article><span className="source-tag">Analyst</span><h3>Required follow-up</h3><p>Run the full retained underwriting engine and complete document, commercial, legal, financing, and accounting diligence before any committee approval.</p></article>{acceptedDrafts.map((proposal) => <article key={proposal.proposalId}><span className="source-tag">Model proposed · accepted by {proposal.humanActor}</span><h3>{proposal.title}</h3><p>{proposal.body}</p></article>)}{reviewed.length ? <article><span className="source-tag">Human review ledger</span><h3>Model proposal disposition</h3><ul>{reviewed.map((proposal) => <li key={proposal.proposalId}>{proposal.title} — {proposal.state.toLowerCase()} by {proposal.humanActor}</li>)}</ul></article> : null}<footer>Draft only · Quick Package · Not investment advice · No model-authored recommendation</footer></section><div className="memo-actions"><button className="primary-button" type="button" onClick={() => window.print()}>Print IC memo</button></div></div>;
}

export function LocalDealShell({result, view, onNavigate, onDeals, onConnect, connection, modelTransport, proposals, onProposalsChange}: {result: IntakeResult; view: DealView; onNavigate: (view: DealView) => void; onDeals: () => void; onConnect: () => void; connection: ConnectionState | null; modelTransport?: ModelTransport; proposals: ModelProposal[]; onProposalsChange: (proposals: ModelProposal[]) => void}) {
  const deal = result.deal!;
  return <div className="product-shell"><aside className="sidebar"><button type="button" className="wordmark" onClick={onDeals}><span>UD</span><strong>Underwriting Desk</strong></button><nav aria-label="Deal navigation">{dealViews.map((item) => <button key={item} type="button" className={item === view ? "active" : ""} aria-current={item === view ? "page" : undefined} onClick={() => onNavigate(item)}>{labels[item]}</button>)}</nav><div className="sidebar-foot"><span className="quiet-chip">Browser local</span><p>Refresh clears this deal.</p></div></aside><div className="shell-main"><header className="deal-topbar"><button type="button" className="mobile-wordmark" onClick={onDeals}>Underwriting Desk</button><strong>{deal.company}</strong><div className="topbar-meta"><span>Growth SaaS Quick Package</span><span>{deal.cutoff}</span></div><ModelConnectionButton connection={connection} onClick={onConnect} /></header><nav className="mobile-nav" aria-label="Deal navigation">{dealViews.map((item) => <button key={item} type="button" className={item === view ? "active" : ""} aria-current={item === view ? "page" : undefined} onClick={() => onNavigate(item)}>{labels[item]}</button>)}</nav><main className="deal-main" id="main-content"><header className="deal-heading"><div><p className="eyebrow">{labels[view]}</p><h1>{deal.company}</h1><p>Browser-local supported package · analyst owner: {deal.analystOwner}</p></div><p className="ic-question"><span>IC question</span>Does the package clear its declared return and runway thresholds?</p></header>{view === "overview" ? <LocalOverview result={result} /> : null}{view === "financials" ? <LocalFinancials result={result} /> : null}{view === "diligence" ? <LocalDiligence result={result} modelTransport={modelTransport} connection={connection} proposals={proposals} onProposalsChange={onProposalsChange} /> : null}{view === "documents" ? <LocalDocuments result={result} /> : null}{view === "memo" ? <LocalMemo result={result} proposals={proposals} /> : null}</main></div></div>;
}
