...
function LocalDiligence({result, state, update, modelTransport, connection}: {result: IntakeResult; state: DealWorkspaceState; update: WorkspaceUpdate; modelTransport?: ModelTransport; connection: ConnectionState | null}) {
  ...
  return <div className="view-stack"><label className="mobile-workspace-selector"><span>Diligence workspace</span><select value={section} onChange={(event) => setSection(event.target.value as typeof section)}>{tabs.map((tab) => <option key={tab.id} value={tab.id}>{tab.label}</option>)}</select></label><nav className="workspace-tabs" aria-label="Diligence workspace">{tabs.map((tab) => <button type="button" key={tab.id} aria-pressed={section === tab.id} onClick={() => setSection(tab.id)}>{tab.label}</button>)}</nav>{section === "issues" ? <DiligenceWorklist state={state} update={update} /> : null}{section === "assumptions" ? <AssumptionRegistry assumptions={assumptions} state={state} update={update} /> : null}{section === "policy" ? <PolicyRegistry profile={analysis.policyProfile} state={state} update={update} blockingGates={analysis.tests.filter((test) => test.blocksAdvancement).map((test) => ({gateId: test.gateId, label: test.label}))} overridableGateIds={LOCAL_OVERRIDABLE_GATES} /> : null}{section === "model" ? <ModelReviewPanel connection={connection} transport={modelTransport} proposals={state.proposals} onProposalsChange={(next) => update((current) => ({proposals: typeof next === "function" ? next(current.proposals) : next}))} evidence={analysis.metrics.map((metric) => ({id: metric.id, title: metric.label, displayValue: metric.display, summary: metric.meaning}))} /> : null}</div>;
}

function LocalDocuments({result}: {result: IntakeResult}) {
  const analysis = result.analysis!; const [query, setQuery] = useState(""); const keyFor = (preview: QuickAnalysis["sourcePreviews"][number]) => `${preview.sourceFile}:${preview.title}:${preview.period}`; const [selected, setSelected] = useState(analysis.sourcePreviews[0] ? keyFor(analysis.sourcePreviews[0]) : "");
  ...
  return <div className="documents-workspace"><section className="document-register"><div className="section-heading"><div><p className="eyebrow">Local package</p><h2>Sources and evidence</h2></div><span>{result.files.filter((file) => file.state === "READY").length} ready</span></div><label className="search-field"><span>Search source excerpts</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="source-list">{previews.map((preview) => <button type="button" aria-pressed={keyFor(preview) === keyFor(active)} className={keyFor(preview) === keyFor(active) ? "active" : ""} key={keyFor(preview)} onClick={() => setSelected(keyFor(preview))}><strong>{preview.title}</strong><small>{preview.sourceFile} · {preview.period}</small><em>{stateLabel(preview.classification)}</em></button>)}</div></section><section className="document-preview">{active ? <><header><div><p className="eyebrow">Evidence preview</p><h2>{active.title}</h2><p>{active.sourceFile} · {active.period}</p></div><span className="status status-recognized">{stateLabel(active.classification)}</span></header><div className="excerpt-list"><article><dl>{active.excerpt.map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl></article></div><aside className="linked-calculations"><h3>Outputs using this source</h3>{analysis.metrics.filter((metric) => metric.sourceFiles.includes(active.sourceFile)).map((metric) => <div className="linked-output" key={metric.id}><span>{metric.label}</span><strong>{metric.display}</strong><small>{metric.meaning}</small></div>)}</aside><details className="technical-record"><summary>Validation receipt</summary><dl>{result.files.filter((file) => file.name === active.sourceFile).map((file) => <div key={file.name}><dt>{file.name}</dt><dd>{file.sha256 ?? file.detail}</dd></div>)}</dl></details></> : <p>No source matches the search.</p>}</section></div>;
}
...
export function LocalDealShell({result, view, onNavigate, onDeals, onConnect, connection, modelTransport, persistenceNotice = ""}: {result: IntakeResult; view: DealView; onNavigate: (view: DealView) => void; onDeals: () => void; onConnect: () => void; connection: ConnectionState | null; modelTransport?: ModelTransport; persistenceNotice?: string}) {
  ...
  return <div className="product-shell">
    <aside className="sidebar"><button type="button" className="wordmark" onClick={onDeals}><span>U</span><strong>Underwriting Desk</strong></button><nav aria-label="Deal navigation">{dealViews.map((item) => <button key={item} type="button" className={item === view ? "active" : ""} aria-current={item === view ? "page" : undefined} onClick={() => onNavigate(item)}>{labels[item]}</button>)}</nav><div className="sidebar-foot"><button type="button" onClick={onConnect}>Model settings</button><span>{persistenceNotice || storageNotice}</span><small>Public synthetic workspace</small></div></aside>
