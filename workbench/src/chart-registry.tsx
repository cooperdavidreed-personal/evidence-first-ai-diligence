...
export function ChartRegistryCaption({caseData, location}: {caseData: CaseData; location: ChartContract["rendered_location"]}) {
  ...
  return <div className="chart-contracts" aria-label={`${location} chart contracts`}>
    {contracts.map((contract) => <aside className="chart-contract" data-chart-id={contract.chart_id} key={contract.chart_id} aria-label={`Chart contract ${contract.chart_id}`}>
      ...
      <dl><div><dt>Point-of-view conclusion</dt><dd>{contract.conclusion}</dd></div><div><dt>Uncertainty</dt><dd>{contract.uncertainty}</dd></div><div><dt>Decision dependency</dt><dd>{contract.decision_dependency}</dd></div></dl>
    </aside>)}
  </div>;
}
