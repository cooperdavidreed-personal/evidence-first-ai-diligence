...
def build(repo: Path, destination: Path) -> None:
    ...
    rows = "\n".join(
        "<tr><td>{}</td><td><strong>{}</strong></td><td>{}</td></tr>".format(
            ...
            html.escape(item["citation_status"]),
...
<section><h2>Failure is a feature</h2><div class="grid"><article><span>Before</span><strong>{html.escape(before["workflow_disposition"])}</strong><p>The material ARR claim has NO_CITATIONS.</p></article><article><span>After evidence</span><strong>{html.escape(after["workflow_disposition"])}</strong><p>Verified citation bytes coexist with counter-citations, temporal blocks, and pending human adjudication.</p></article></div></section>
<section><h2>Claim ledger</h2><div class="table-wrap" tabindex="0" role="region" aria-label="Claim ledger, horizontally scrollable on small screens"><table><caption>Citation statuses for the corrected synthetic case</caption><thead><tr><th scope="col">ID</th><th scope="col">Citation status</th><th scope="col">Claim</th></tr></thead><tbody>{rows}</tbody></table></div></section>
